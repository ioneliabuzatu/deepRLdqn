...
def run_episode(ort_session, env_seed):
...
if __name__ == "__main__":
    ...
    param_file = args.submission

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Load submission
    ort_session = onnxruntime.InferenceSession(param_file)

    seed = 0  # np.loadtxt('/daten/challenge/django/seed.rnd', dtype=np.int)
    np.random.seed(seed)

    scores = []
    for i in range(episodes):
        env_seed = np.random.randint(1000)
        scores.append(run_episode(ort_session, env_seed))
